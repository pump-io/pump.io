...
User.prototype.beforeUpdate = function(props, callback) {
    ...
    if (!_(props).has("password") && !_(props).has("email")) {
        callback(new Error("'password' or 'email' property is required"), null);
        return;
    }

    if (_(props).has("password") && User.isBadPassword(props.password)) {
        callback(new User.BadPasswordError(), null);
        return;
    }

    // Update pending email if has one

    if (_(props).has("email") && (this.email_pending || props.email !== this.email)) {
        props.email_pending = props.email;
        delete props.email;
    } else if (_(props).has("email_pending")) {
        // Callers must omit or leave equal

        if (props.email_pending !== this.email_pending) {
            callback(new Error("'email_pending' is immutable"), null);
            return;
        } else {
            delete props.email_pending;
        }
    }
