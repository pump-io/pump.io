...
var PumpWeb = _.extend(_.clone(PumpApp), {
    ...
    initApp: function(app) {
        ...
        app.post('/main/login', this.noUser, this.handleLogin);
        app.post('/main/logout', this.mustAuth, this.handleLogout);

        app.get('/main/settings', this.mustAuth, this.showSettings);
        app.post('/main/settings', this.mustAuth, this.saveSettings);

        app.get('/:nickname', this.maybeAuth, this.reqUser, this.showStream);
        app.get('/:nickname/inbox', this.mustAuth, this.reqUser, this.sameUser, this.showInbox);
        app.get('/:nickname/activity/:uuid', this.maybeAuth, this.reqUser, this.showActivity);
    },

    showSettings: function(req, res) {
        var pump = this;
		...
		// FIXME: sidebar-less page
                pump.runTemplate("settings-sidebar", req.remoteUser, this.parallel());
            },
    ...
    mustAuth: function(req, res, next) {
	    ...
	    pump.showError(res, new Error("No logged-in user"));
        } else {
            Step(
                function() {
