...
var activityFeed = function(urlmaker, titlemaker, streammaker, finisher) {
    ...
    return function(user, principal, args, callback) {
        ...
        Step(
            ...
            function(err) {
                if (err) throw err;
                if (finisher) {
                    finisher(principal, collection.items, this);
                } else {
                    this(null);
                }
            },
    ...
    return function(user, principal, args, callback) {
        ...
        Step(
            ...
            function(err, people) {
                ...
                finisher(principal, people, this);
            },
            function(err) {
    ...
    return function(context, principal, args, callback) {
        ...
        Step(
            ...
            function(err, results) {
                ...
                if (!principal) {
                    // XXX: keep a separate stream instead of filtering
                    str = new FilteredStream(str, objectPublicOnly);
                } else {
                    str = new FilteredStream(str, objectRecipientsOnly(principal));
                }
...
var userFollowers = personFeed(
    ...
    function(user, principal, collection, callback) {
...
var userFavorites = objectFeed(
    ...
    function(principal, items, callback) {
        Step(
            function() {
                // Add the first few replies for each object
                firstFewReplies(principal, items, this.parallel());

                // Add the first few replies for each object

                firstFewShares(principal, items, this.parallel());

                // Add the first few "likers" for each object

                addLikers(principal, items, this.parallel());

                // Add the shared flag for each object

                addShared(principal, items, this.parallel());

                // Add the liked flag for each object

                addLiked(principal, items, this.parallel());

                // Add the proxy URLs for each object

                addProxyObjects(principal, items, this.parallel());
            },
            callback
        );
    }
...
var userUploads = objectFeed(
    ...
    function(principal, collection, callback) {
        callback(null);
    });

var objectLikes = objectFeed(
    function(context) {
    ...
    function(principal, items, callback) {
        Step(
            function() {
                ...
                followable = _.filter(items, function(obj) {
