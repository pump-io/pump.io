...
var http = require("http"),
    ...
    // Required becasue stupid Node 0.10 doesn't support ES6 octal literals
    octal = require("octal");
...
var getJSON = function(serverUrl, cred, callback) {
    ...
    var oa = newOAuth(serverUrl, cred);
...
var clientCred = function(host, callback) {
    /*
            cred will look like when read from the file.
                 {
                    client_id: "XXXX",
                    client_secret: "YYYY",
                    expires_at: 0
...
var ensureDir = function(dirName, callback) {
    Step(
        ...
        function(err, stat) {
            if (err) {
                if (err.code === "ENOENT") {
                    fs.mkdir(dirName, octal(700), this);
                } else {
                    throw err;
                }
