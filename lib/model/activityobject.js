...
ActivityObject.ensureObject = function(obj, callback) {
    ...
    var type = obj.objectType,
        ...
        id = obj.id,
        url = obj.url,
        getRemoteObject = function(url, cb) {
            ...
            Step(
                ...
                function(err, body, resp) {
                    ...
                    if (resp.statusCode != 200) {
                        throw new Error("Bad status code: " + resp.statusCode);
                    }
        ...
        mergeLinks = function(jrd, obj) {
            var feeds = ["followers", "following", "links", "favorites"];
        ...
        tryDiscover = function(obj, cb) {
            ...
            Step(
                ...
                function(err, jrd) {
                    ...
                    if (selfies.length > 0) {
                        getRemoteObject(selfies[0].href, this);
                    } else {
                        this(null, obj);
                    }
                },
    ...
    Step(
        ...
        function(err, result) {
            var delta;
            if (err) throw err;
            if (!result && !ActivityObject.isLocal(obj)) {
