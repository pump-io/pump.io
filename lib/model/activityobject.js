...
ActivityObject.beforeCreate = function(props, callback) {
    ...
    if (!_(props).has("id")) {
        ...
        props.id   = ActivityObject.makeURI(this.type, props._uuid);

        // XXX: fall back to User.fromPerson() if needed
        ...
        if (_.has(props, "author") &&
            ...
            props.url = URLMaker.makeURL([props.author.preferredUsername, this.type, props._uuid].join("/"));
        }
        ...
        props.links.self = {
            href: URLMaker.makeURL("api/" + this.type + "/" + props._uuid)
        };

        props.likes = {
            url: URLMaker.makeURL("api/" + this.type + "/" + props._uuid + "/likes")
        };

        props.replies = {
            url: URLMaker.makeURL("api/" + this.type + "/" + props._uuid + "/replies")
        };
...
ActivityObject.prototype.expandFeeds = function(callback) {
    ...
    Step(
        ...
        function(err, repliesCount, favoritersCount) {
            if (err) {
                callback(err);
            } else {
                if (obj.replies) {
