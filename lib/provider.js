    ...
    generateAccessToken: function(oauthToken, callback) {
        ...
        Step(
            ...
            function(err, ats) {
                ...
                if (err) throw err;
                if (!ats || ats.length === 0) {
                    if (provider.log) provider.log.info("creating a new access token for " + oauthToken);
                    props = {consumer_key: rt.consumer_key,
                             request_token: rt.token,
                             username: rt.username};
                    AccessToken.create(props, this);
                } else {
                    if (provider.log) provider.log.info("reusing access token " + ats[0].access_token + " for " + oauthToken);
            ...
            function(err, results) {
                ...
                at = results;
                // XXX: delete...?
                if (provider.log) provider.log.info("saving access token for " + oauthToken);
