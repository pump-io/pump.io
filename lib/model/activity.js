...
Activity.makeURI = function(type, uuid) {
    return Activity.makeURL('api/' + type + '/' + uuid);
};

Activity.makeURL = function(rel) {
    // Crazy syntax to avoid circular require
    return require('../pumpapi').PumpAPI.makeURL(rel);
};
...
Activity.prototype.save = function(callback) {
    ...
    if (!act.id) {
        ...
        act.links = {};
        act.links.self = Activity.makeURL('api/activity/' + act.uuid);
        // FIXME: assumes person data was set and that it's a local actor
        act.url  = Activity.makeURL(act.actor.preferredUsername + '/activity/' + act.uuid);
    }
...
Activity.prototype.expandObject = function(callback) {
    ...
    Activity.getObject(this.object.objectType, this.object.id, function(err, object) {
        if (err) {
            callback(err, null);
        } else {
