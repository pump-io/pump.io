...
User.beforeCreate = function(props, callback) {
    ...
    Step(
        ...
        function(err, person) {
            ...
            Stream.create({name: "user:" + props.nickname + ":lists"}, this.parallel());
        },
        function(err, inbox, outbox, followers, following, favorites, lists) {
            var list,
                defaults = ["Friends", "Family", "Acquaintances", "Coworkers", "Following"],
                i,
                group = this.group();
            ...
            for (i = 0; i < defaults.length; i++) {
                Collection.create({author: props.profile, displayName: defaults[i], objectTypes: ["person"]}, group());
            }
