...
var _ = require("underscore"),
    ...
    Ignore = require("../lib/model/ignore").Ignore,
...
// finisher that adds ignored flag to stuff

var addIgnoreFinisher = function(principal, collection, callback) {
    ...
    addIgnore(principal, _.pluck(collection.items, "object"), callback);
};

var addIgnore = function(profile, objects, callback) {
    ...
    ignoreIDs = objects.map(function(object) {
        return Ignore.id(profile.id, object.id);
    });

    Step(
        function() {
            Ignore.readAll(ignoreIDs, this);
        },
        function(err, ignores) {
            if (err) {
                ...
                _.each(objects, function(object, i) {
                    var ignoreID = ignoreIDs[i];

                    if (_.has(ignores, ignoreID) && _.isObject(ignores[ignoreID])) {
                        object.ignore = true;
                    } else {
                        object.ignore = false;
                    }
...
exports.addIgnoreFinisher = addIgnoreFinisher;
