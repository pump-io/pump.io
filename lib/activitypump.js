...
var ActivityPump = {
            ...
            if (err) {
                ...
                activity.save(function(err, results) {
                    if (err) {
                        ...
                        ActivityPump.addToOutbox(activity, req.person, function(err) {
                            if (err) {
                                ActivityPump.showError(res, err);
                            } else {
                                // ...then distribute.
    ...
    addToOutbox: function(activity, person, next) {
        var bank = ActivityPump.bank;
        bank.prepend('outbox', person.id, activity.id, function(err, value) {
            if (err) {
                next(err);
            } else {
                bank.incr('outboxcount', person.id, function(err, value) {
                    if (err) {
                        next(err);
                    } else {
                        next(null);
    ...
    userFeed: function(req, res, next) {
        ...
        var start, cnt, end, bank = ActivityPump.bank;

        start = (req.query.offset) ? parseInt(req.query.offset) : 0;
        cnt = (req.query.cnt) ? parseInt(req.query.cnt) : ActivityPump.DEFAULT_ACTIVITIES;
        end = start + cnt;

        bank.read('outboxcount', req.user.personId, function(err, totalOutbox) {
            if (err) {
                ActivityPump.showErr(res, err);
            } else {
                collection.totalCount = totalOutbox;
                bank.slice('outbox', req.user.personId, start, end, function(err, activityIds) {
                    if (err) {
                        if (err instanceof NoSuchThingError) {
                            collection.items = [];
                            ActivityPump.showData(res, collection);
                        ...
                        ActivityPump.bank.readAll('activity', activityIds, function(err, activityMap) {
                            var activities = [], id;
                            if (err) {
                                ActivityPump.showError(res, err);
                            } else {
    ...
    userInbox: function(req, res, next) {
        ...
        ActivityPump.bank.read('inboxcount', req.user.personId, function(err, inboxCount) {
            if (err) {
                ...
                ActivityPump.bank.slice('inbox', req.user.personId, start, end, function(err, inbox) {
                    if (err) {
                        // No inbox yet; show empty collection
                        if (err instanceof NoSuchThingError) {
                            ActivityPump.showData(res, collection);
