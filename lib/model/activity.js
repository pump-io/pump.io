...
Activity.prototype.save = function(callback) {
    ...
    if (!this.id) {
        this.uuid = Activity.newId();
        this.id   = Activity.makeURI('activity', this.uuid);
    }

    Activity.ensureObject(this.actor, function(err, actor) {
	if (err) {
	    callback(err);
	} else {
	    Activity.ensureObject(this.object, function(err, object) {
		if (err) {
		    callback(err);
		} else {
		    // slim them down to references
		    this.actor = {objectType: actor.objectType,
				  id: actor.id};
		    this.object = {objectType: object.objectType,
				   id: object.id};
		    this.defaultSave(callback);
		}
	    });
	}
    });
...
Activity.get = function(id, callback) {
	...
	if (err) {
	    ...
	    Activity.getObject(activity.actor.objectType, activity.actor.id, function(err, actor) {
		if (err) {
		    callback(err, null);
		} else {
		    activity.actor = actor;
		    Activity.getObject(activity.object.objectType, activity.object.id, function(err, object) {
			if (err) {
			    callback(err, null);
			} else {
