...
var upgradeGroup = function(group, callback) {
    ...
    Step(
	function() {
	    upgradeGroupFeed(group, "members", this.parallel());
	    upgradeGroupFeed(group, "documents", this.parallel());
	},
	function(err) {
	    callback(err);
	}
...
var upgradeGroupFeed = function(group, feed, callback) {
	...
	function(err, isLocal) {
		...
		group[feed] = {
		    url: URLMaker.makeURL("api/group/"+group._uuid+"/"+feed)
		};
		group.save(this);
	    }
	},
