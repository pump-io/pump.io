    ...
    return function(user, principal, args, callback) {
        ...
        Step(
            ...
            function(err, people) {
                ...
                finisher(user, principal, collection, this);
            },
            function(err) {
    ...
    function(context, principal, collection, callback) {
        Step(
            function() {
                ...
                firstFewReplies(principal, collection, this.parallel());

                // Add the first few shares for each object

                firstFewShares(principal, collection, this.parallel());

                // Add the first few "likers" for each object

                addLikers(principal, collection, this.parallel());

                // Add whether the current user likes the collection

                addLiked(principal, collection, this.parallel());

                // Add the followed flag to applicable objects

                followable = _.filter(collection, function(obj) {
                    return obj.isFollowable();
                });

                addFollowed(principal, followable, this.parallel());

                addProxyObjects(principal, collection, this.parallel());
            },
            callback
        );
    }
