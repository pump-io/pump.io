...
var makeApp = function(configBase, callback) {
    ...
    /*
        ...
        We set this up here (using configBase) as well as in bin/pump
        for a couple reasons:

        1. It makes sure the tests get a config with all the defaults

        2. Using configBase instead of an already-populated config
        means *this* call doesn't throw because of already-populated
        internal params

        3. Building and validating config in bin/pump ensures that
        bin/pump throws instead of 4 worker processes, which will be
        infinitely respawned

        It's a little less efficient, but every other way is terrible
        so who cares.

    */
    config = configUtil.buildConfig(configBase);
    ...
    db.connect({}, function(err) {
        ...
        if (config.smtpserver) {
            // harmless flag
            // XXX does this belong in config.js?
        ...
        app.set("views", [
            ...
            path.resolve(__dirname, "../public/template/lib")
        ]);
        ...
        if (config.noweb) {
            // Don't serve content of `javascript` or `template` directories in noweb.
            // This is before `express.static()` so we can override its behavior.
            app.use(function(req, res, next) {
                var path = req.originalUrl.split("/")[1];

                if (path.match(/javascript|template/)) {
