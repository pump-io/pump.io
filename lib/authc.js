...
var userAuth = function(req, res, next) {
    ...
    areq.authenticate(["user"], function(error, authenticated) {
        ...
        if (req.app.config.requireEmail && !deetz.user.user.email &&
            !(req.method === "PUT" && deetz.user.user.email_pending &&
              req.path.match(/^\/api\/user\/[a-zA-Z0-9\-_.]{1,64}$/))) {
            // TODO: put username regex in a constants file
