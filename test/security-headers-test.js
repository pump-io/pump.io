    ...
    "When we setup the app": {
        ...
        "and we HEAD the home page": {
            ...
            "the Content-Security-Policy header is correct": function(err, res, body) {
                ...
                assert.equal(res.headers["content-security-policy"], "default-src 'self'; connect-src 'self' ws://localhost:4815; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src *; object-src 'none'; media-src *; child-src 'self' www.youtube.com; frame-ancestors 'none'");
            },
