...
User.prototype.afterCreate = function(callback) {
    ...
    var user = this,
        ...
        createLists = function(callback) {
            Step(
                function() {
                    Stream.create({name: "user:" + user.nickname + ":lists:person"}, this);
                },
                function(err, str) {
                    var i,
                    ...
                    for (i = 0; i < lists.length; i++) {
                        Collection.create({author: user.profile, displayName: lists[i], objectTypes: ["person"]}, group());
                    }
                },
                callback
            );
            ...
            Step(
                function() {
                    ...
                    _.each(rels, function(name, rel) {
                        var id = URLMaker.makeURL("/api/"+user.nickname+"/"+rel),
    ...
    Step(
        function() {
            ...
            createLists(this.parallel());
            createGalleries(this.parallel());
            createVirtualLists(this.parallel());
        },
        function(err, person, streams, lists, galleries, virtuals) {
            if (err) {
                callback(err);
            } else {
                callback(null);
...
User.prototype.uploadsStream = function(callback) {
    var user = this;
    Stream.get("user:" + user.nickname + ":favorites", callback);
};
