...
Activity.prototype.apply = function(defaultActor, callback) {
        ...
        Step(
            ...
            function(err, toDelete) {
                ...
                if (toDelete.author.id !== act.actor.id) {
                    throw new Error("Can't delete " + toDelete.id + ": not author.");
                }
        ...
        Step(
            ...
            function(err, toUpdate) {
                ...
                if (toUpdate.author.id !== act.actor.id) {
                    throw new Error("Can't update " + toUpdate.id + ": not author.");
                }
    ...
    case Activity.ADD:
        Step(
            function() {
                ActivityObject.getObject(act.object.objectType, act.object.id, this.parallel());
            ...
            function(err, toAdd, target) {
                ...
                if (target.author.id !== act.actor.id) {
                    throw new Error("Can't add to " + target.id + ": not author.");
                }
                if (target.objectType !== "collection") {
                    throw new Error("Can't add to " + target.id + ": not a collection.");
                }
                if (!_(target).has('objectTypes') || !_(target.objectTypes).isArray() ||
                    target.objectTypes.indexOf(toAdd.objectType) === -1) {
                    throw new Error("Can't add to " + target.id + ": incorrect type.");
                }
    ...
    case Activity.REMOVE:
        Step(
            ...
            function(err, toAdd, target) {
                if (err) throw err;
                if (target.author.id !== act.actor.id) {
                    throw new Error("Can't remove from " + target.id + ": not author.");
                }
                if (target.objectType !== "collection") {
                    throw new Error("Can't remove from " + target.id + ": not a collection.");
                }
                if (!_(target).has('objectTypes') || !_(target.objectTypes).isArray() ||
                    target.objectTypes.indexOf(toAdd.objectType) === -1) {
                    throw new Error("Can't remove from " + target.id + ": incorrect type.");
                }
                target.getStream(this);
            },
            function(err, stream) {
