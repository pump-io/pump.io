...
User.prototype.follow = function(id, callback) {
    ...
    Step(
        function() {
            Edge.create({from: user.profile.id, to: id}, this);
        },
        function(err, edge) {
            if (err) throw err;
...
User.prototype.stopFollowing = function(id, callback) {
    ...
    Step(
        function() {
            Edge.search({from: user.profile.id, to: id}, this);
        },
        function(err, edges) {
            if (err) throw err;
            if (edges.length === 0) {
                throw new NoSuchThingError('edge', user.profile.id + '/' + id);
            }
            edges[0].del(this);
        },
        function(err) {
