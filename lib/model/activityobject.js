...
ActivityObject.beforeCreate = function(props, callback) {
    ...
    Step(
        ...
        function(err) {
            if (err) throw err;
            ActivityObject.validate(props, this);
        },
        function(err) {
            if (err) {
                callback(err, null);
...
ActivityObject.ensureObject = function(obj, callback) {
    ...
    if (!_.isString(type)) {
        callback(new TypeError("Type is not a string: " + id), null);
        return;
    }
...
ActivityObject.validate = function(props, callback) {

    // Validate attachments

    if (_.has(props, "attachments")) {
        if (!_.isArray(props.attachments)) {
            callback(new TypeError("attachments is not an array"));
            return;
        }
        try {
            _.each(props.attachments, function(attachment) {
                ...
                ActivityObject.validate(attachment, function(err) {
                    if (err) throw err;
                });
            });
        } catch (err) {
    ...
    if (!_.isString(props.id)) {
        callback(new TypeError("no id in activity object"));
        return;
    }
    ...
    if (!_.isString(props.objectType)) {
        callback(new TypeError("no objectType in activity object"));
        return;
    }

    if (_.has(props, "author")) {
        if (!_.isObject(props.author)) {
            callback(new TypeError("author property is not an activity object"));
            return;
            ActivityObject.validate(props.author, function(err) {
                callback(err);
                return;
            });
        }
    ...
    if (_.has(props, "content") && !_.isString(props.content)) {
        callback(new TypeError("content property is not a string"));
        return;
    }
    ...
    if (_.has(props, "displayName") && !_.isString(props.displayName)) {
        callback(new TypeError("displayName property is not a string"));
        return;
    }

    if (_.has(props, "downstreamDuplicates")) {
        if (!_.isArray(props.downstreamDuplicates)) {
            callback(new TypeError("downstreamDuplicates is not an array"));
            return;
        }

        // XXX: validate that duplicates are URIs

        if (_.some(props.downstreamDuplicates, function(dupe) { return !_.isString(dupe); })) {
            callback(new TypeError("downstreamDuplicates member is not a string"));
            return;
        }
    }
    ...
    if (_.has(props, "image")) {
        if (!_.isObject(props.image)) {
            callback(new TypeError("image property is not an object"));
            return;
        }
    }
