    ...
    for (i = 0; i < props.length; i++) {
        ...
        if (addrs && _.isArray(addrs)) {
            for (j = 0; j < addrs.length; j++) {
                if (_.isObject(addrs[j])) {
                    addrs[j] = ActivityObject.toObject(addrs[j], ActivityObject.PERSON);
                }
            }
        }
    }
...
Activity.prototype.sanitize = function(user) {
    ...
    var act = this, i, j, props = ["to", "cc", "bto", "bcc"];
    ...
    if (this.actor && this.actor.sanitize) {
        this.actor.sanitize();
    }

    if (this.object && this.object.sanitize) {
        this.object.sanitize();
    }

    for (i = 0; i < props.length; i++) {
        if (this[props[i]]) {
            for (j = 0; j < this[props[i]].length; j++) {
                if (this[props[i]][j].sanitize) {
                    this[props[i]][j].sanitize();
                }
            }
        }
    }
