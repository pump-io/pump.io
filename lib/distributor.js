...
var path = require("path"),
    ...
    tview = jade.compileFile(path.join(__dirname, "..", "public", "template", "lib", "activity-notification-text.jade")),
    hview = jade.compileFile(path.join(__dirname, "..", "public", "template", "lib", "activity-notification-html.jade"));
...
var Distributor = function(activity) {
    ...
    var dtor = this,
        ...
        derror = function(err, obj, message) {
            ...
            if (Distributor.log) {
                Distributor.log.error(obj, message);
            }
        },
        delivered = {},
        expanded = false,
            ...
            Step(
                ...
                function(err, cnt) {
                    ...
                    str.each(
                        ...
                        function(err) {
                            dinfo({expected: cnt, actual: counter, activity: activity.id}, "Delivery metrics");
                            cb(err);
                        }
                    );
                },
            ...
            Step(
                ...
                function(err, inbox, members) {
                    ...
                    if (group._foreign_id) {
                        sendUpdate(URLMaker.makeURL("/api/group/inbox", {id: group.id}));
                    }
                    ...
                    members.each(
                        function(id, callback) {
                            Person.get(id, function(err, person) {
                                if (err) {
                                    // Log and continue
                                    derror(err, {person: id}, "Error getting group member");
                                    callback(null);
                                } else {
        ...
        inboxUpdates = function(user) {
            ...
            var isDirectTo = function(user) {
                ...
                return _.some(recipients, function(item) {
        ...
        notifyByEmail = function(user, activity, callback) {
            ...
            try {
                html = hview({principal: user.profile, principalUser: user, activity: activity});
                text = tview({principal: user.profile, principalUser: user, activity: activity});
            } catch (err) {
                callback(err, null);
                return;
            }
        ...
        var actor = activity.actor,
            ...
            toEmail = function(cb) {
                var direct = directRecipients(activity),
                    people = _.filter(direct, {objectType: ActivityObject.PERSON});
            ...
            toFirehose = function(cb) {
                ...
                var recipients = activity.recipients(),
                    pub = _.filter(recipients, {id: Collection.PUBLIC});
            ...
            function(err) {
                ...
                _.each(Distributor.plugins, function(plugin) {
                    try {
                        ...
                        derror(err, {plugin: plugin}, "Error with plugin");
                    }
                });
            },
            callback
