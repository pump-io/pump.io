...
ActivityObject.validate = function(props) {
    ...
    if (_.has(props, "author")) {
        if (!_.isObject(props.author)) {
            throw new TypeError("author property is not an activity object");
            ActivityObject.validate(props.author);
        }
    }

    if (_.has(props, "content") && !_.isString(props.content)) {
        throw new TypeError("content property is not a string");
    }
    ...
    if (_.has(props, "downstreamDuplicates")) {
        if (!_.isArray(props.downstreamDuplicates)) {
            throw new TypeError("downstreamDuplicates is not an array");
        }

        // XXX: validate that duplicates are URIs

        if (_.some(props.downstreamDuplicates, function(dupe) { return !_.isString(dupe); })) {
            throw new TypeError("downstreamDuplicates member is not a string");
        }
    }
...
ActivityObject.validateMediaLink = function(props) {
    ...
    if (_.has(props, "width") && !_.isNumber(props.width)) {
        throw new TypeError("width property of media link is not a number");
    }

    if (_.has(props, "height") && !_.isNumber(props.height)) {
        throw new TypeError("height property of media link is not a number");
    }
};
