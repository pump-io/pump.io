...
Nonce.makeKey = function(consumer_key, access_token, nonce, timestamp) {
    if (access_token) {
        return consumer_key + '/' + access_token + '/' + timestamp + '/' + nonce;
    } else {
        return consumer_key + '/' + timestamp + '/' + nonce;
    }
};
...
Nonce.seenBefore = function(consumer_key, access_token, nonce, timestamp, callback) {
    ...
    var key = Nonce.makeKey(consumer_key, access_token, nonce, timestamp);
    ...
    Step(
        ...
        function(err, found) {
            ...
            if (err && (err instanceof NoSuchThingError)) { // database miss
                ...
                throw err;
            } else if (now() - found.timestamp > TIMELIMIT) { // Out of date
                found.update({timestamp: parseInt(timestamp, 10)}, this);
            } else {
                callback(null, true);
            }
        },
