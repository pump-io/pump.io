    ...
    function(context, principal, collection, callback) {
        ...
        if (collection.items.length > 0) {
            ...
            if (!collection.objectTypes ||
                ...
                collection.items[0].objectType != collection.objectTypes[0]) {
                prevParams.type = collection.items[0].objectType;
            }
            ...
            nextParams = {before: collection.items[collection.items.length - 1].id};

            if (!collection.objectTypes ||
                collection.objectTypes.length != 1 ||
                collection.items[collection.items.length - 1].objectType != collection.objectTypes[0]) {
                nextParams.type = collection.items[collection.items.length - 1].objectType;
            }
