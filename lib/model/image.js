...
// Part of a no-upgrade upgrade. If we get an image that's been saved
// without a thumbnail, add the thumbnail and save.

Image.prototype.afterGet = function(callback) {
    var img = this;

    if (_.isObject(img.image) && !_.has(img.image, "width")) {
        Step(
            function() {
                thumbnail.addImageMetadata(img, this);
            },
            function(err) {
                if (err) throw err;
                img.save(this);
