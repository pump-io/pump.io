...
ActivityObject.beforeCreate = function(props, callback) {
    ...
    if (_(props).has('author')) {
        ActivityObject.ensureObject(props.author, function(err, author) {
            if (err) {
                callback(err, null);
            } else {
                props.author = {id: author.id, objectType: author.objectType};
                callback(null, props);
            }
        });
    } else {
...
ActivityObject.prototype.afterGet = function(callback) {
    this.expandAuthor(callback);
};

ActivityObject.prototype.expandAuthor = function(callback) {
    var obj = this;
    if (_(this).has('author')) {
        ActivityObject.getObject(this.author.objectType, this.author.id, function(err, author) {
            if (err) {
                callback(err);
            } else {
                obj.author = author;
...
ActivityObject.prototype.beforeSave = function(callback) {
    ...
    if (_(this).has('author')) {
        ActivityObject.ensureObject(this.author, function(err, author) {
            if (err) {
                callback(err);
            } else {
                this.author = {id: author.id, objectType: author.objectType};
                callback(null);
            }
        });
    } else {
