...
/*
  relmaker returns the function that takes a user context that contains the user name
  and returns a url.
  titlemake returns the title of the stream.
  streammaker produces the stream
  finisher does the final processing
*/
        ...
        Step(
            ...
            function(err, ids) {
                ...
                if (ids.length === 0) {
                    ...
                    console.log("read people ids:")
                    console.log(ids) // null

                    ids2 = new Array(ids.length);
...
var userFollowing = personFeed(
    function(context) {// relmaker
        var user = context.user;
        return "api/user/" + user.nickname + "/following";
    },
    function(context) {//titlemake
        var user = context.user;
        return "Following for " + (user.profile.displayName || user.nickname);
    },
    function(context, callback) {//streammaker
        var user = context.user;
        user.followingStream(callback);
    },
    function(context, principal, collection, callback) { // finisher
        var user = context.user;
        collection.author = user.profile;
        if (collection.author) {
            collection.author.sanitize();
...
var objectReplies = objectFeed(
    function(context) {
        var objectType = context.objectType,
            obj = context.obj;
        return "api/" + objectType + "/" + obj._uuid + "/replies";
    },
    function(context) {
        var obj = context.obj;
