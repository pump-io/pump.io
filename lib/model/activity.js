...
Activity.prototype.apply = function(defaultActor, callback) {
    ...
    if (!_.includes(Activity.verbs, verb)) {
        callback(null);
        return;
    }
...
Activity.beforeCreate = function(props, callback) {
    ...
    Step(
        function() {
            var group = this.group();
            _.each(Activity.oprops, function(prop) {
                ActivityObject.ensureProperty(props, prop, group());
            });
            _.each(Activity.aprops, function(prop) {
...
Activity.prototype.toAS2 = function() {
    // XXX handle subobjects

    // XXX can we get away with _.clone?
    var as2 = _.cloneDeep(this);
...
Activity.validate = function(props) {
    ...
    _.each(Activity.oprops, function(name) {
        if (_.has(props, name)) {
    ...
    _.each(Activity.aprops, function(name) {
        if (_.has(props, name)) {
